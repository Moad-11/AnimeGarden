...
const FALLBACK_NEWS = [
    {
        ...
        fullContent: `<div class="news-detail">
...
async function loadRealNews() {
    ...
    try {
        // استخدام الأخبار المعدة مسبقاً مباشرة
...
function displayNews(news) {
    if (!news || news.length === 0) {
        newsContainer.innerHTML = `<div class="no-news">
    ...
    const newsHTML = news.map(item => `<article class="news-card" data-news-id="${item.id}">
